...
## 2026-08-28 - Unredacted StreamGrabError Message and SuggestedAction Leaked Signed URLs and Auth Tokens
**Vulnerability:** `StreamGrabError` redacted `detail` strings but allowed unredacted URLs with tokens and inline authorization headers in `message` and `suggestedAction`, leaking sensitive auth tokens into UI reports, IPC payloads, and log entries.
**Learning:** Error classes that store user messages and error details must redact all string properties during construction before storing or passing them to `super()`.
**Prevention:** Apply `redactText` to all human-readable message and action fields in base error classes.
