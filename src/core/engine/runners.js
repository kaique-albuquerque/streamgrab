...
// ---------------------------------------------------------------------------
// P12: Subtitle embedding via FFmpeg (para fontes nao-ytdlp)
// ---------------------------------------------------------------------------

/**
 * Baixa e embute legendas em um arquivo de video ja baixado.
 *
 * @param {object} params
 * @param {string} params.videoPath — caminho do video final
 * @param {Array<{uri: string, language: string, name: string}>} params.subtitleTracks — tracks a baixar
 * @param {string[]} params.selectedLanguages — idiomas selecionados pelo usuario
 * @param {boolean} params.embedSubs — se true, hardcode legendas no video
 * @param {object} [params.headers] — headers HTTP para download das legendas
 * @param {AbortSignal} [params.signal]
 * @param {Function} [params.onLog]
 * @returns {Promise<{ok: boolean, error?: string}>}
 */
export async function embedSubtitles({ videoPath, subtitleTracks, selectedLanguages, embedSubs = false, headers = {}, signal, onLog }) {
  ...
  if (!Array.isArray(selectedLanguages) || selectedLanguages.length === 0) {
    return { ok: true }; // no languages selected
  }

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sg-subs-'));
  try {
    // Filter to only tracks the user selected (or 'all')
    const wantAll = selectedLanguages.includes('all');
    const wanted = subtitleTracks.filter((t) =>
      wantAll || selectedLanguages.includes(t.language)
    );
    ...
    if (!embedSubs) {
      // Just copy subtitle files next to the video (soft subs)
      const videoDir = path.dirname(videoPath);
      const videoBase = path.basename(videoPath, path.extname(videoPath));
      for (const sub of subFiles) {
        const destName = `${videoBase}.${sub.language}${path.extname(sub.file)}`;
    ...
    // Hardcode subtitles via FFmpeg (-vf subtitles=...)
    // Build complex filter for multiple subtitle tracks
    const inputs = ['-i', videoPath];
    const filters = [];

    for (let i = 0; i < subFiles.length; i++) {
      inputs.push('-i', subFiles[i].file);
      // Escape special chars in path for FFmpeg filter
      const escapedPath = subFiles[i].file.replace(/'/g, "\\'").replace(/:/g, '\\:');
    ...
    try {
      ...
      if (!result.ok) {
        ...
        // Fallback: save as soft subs next to video
        const videoDir = path.dirname(videoPath);
        const videoBase = path.basename(videoPath, path.extname(videoPath));
        for (const sub of subFiles) {
          const destName = `${videoBase}.${sub.language}${path.extname(sub.file)}`;
          await fs.promises.copyFile(sub.file, path.join(videoDir, destName)).catch(() => {});
        }
        return { ok: true }; // soft subs saved
      }
