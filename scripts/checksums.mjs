 ...
 *
 * Varre dist/ por instaladores (.msi, .pkg, .AppImage e .deb) e escreve
...
export function main() {
  ...
  const artifacts = fs
    ...
    .filter((name) => /\.(exe|msi|dmg|pkg|zip|AppImage|deb|blockmap|yml)$/i.test(name))
    .sort();

  if (!artifacts.length) {
    throw new Error(`Nenhum artefato em ${DIST_DIR} (esperava instaladores de Windows, macOS ou Linux).`);
  }
